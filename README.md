...
조각을 등록할 때 이미지를 첨부해주어야 합니다. 이미지 파일을 서버에 보내주는 방법을 잘 알지 못한 채 JSON으로 보내 서버에 보내지지 않았고, 이 방법이 잘못되었음을 알고 파일을 서버에 보낼 수 있는 방법에 대해 공부하였습니다. formData를 사용하면 파일과 함께 데이터를 보낼 수 있음을 알게되어서 formData 객체를 만들어준 다음 데이터를 append하여 서버에 post로 보내주었습니다.  

카카오톡 로그인을 구현할 때 로그인 버튼을 눌렀을 때 이동하는 url에 리다이렉트 url을 넣어주어야 했습니다. 이때 저는 서버로 리다이렉트를 보내면 되겠다고 생각했는데 잘되지 않아서 플로우를 다시 찬찬히 이해하려고 해보았습니다. 리다이렉트 url을 서버 url로 설정하는 것이 아니라 프론트엔드에서 한단계 더 처리해주어야 함을 인지하고 그 부분을 수정하였습니다. 로그인 버튼을 누르면 카카오 로그인 동의 페이지로 이동하며, 동의를 눌렀을 때 프론트엔드에서 따로 만들어둔 리다이렉트 페이지로 이동시켰고, 해당 리다이텍트 페이지 url에는 인가 코드가 포함되어 있습니다. 리다이렉트 페이지가 렌더링되는 시점에 서버에서 제공한 url로 인가코드와 함께 요청을 보내면 토큰을 받을 수 있고 이를 쿠키에 저장하였습니다. 

# 아쉬운점

이번 프로젝트는 팀빌딩과 기획에 시간이 많이 소요되었는데 팀장으로서 일정 관리를 더 효율적으로 했으면 좋았겠다는 생각을 하였습니다. 개발을 시작할 때, 클라이언트 서버 각자의 데드라인을 정해서 계획을 세웠으나 예상치 못한 에러와 개발이 지연이 되었던 점이 아쉬웠습니다. 다음에 프로젝트를 진행할때는 좀 더 매끄러운 진행을 위해서 먼저 기획, 서버와의 인터페이스 확립이 제대로 이루어져야겠다는 생각과, 개발 일정을 난이도를 고려하여 잘 설정해야겠다는 생각을 하게되었습니다. 
